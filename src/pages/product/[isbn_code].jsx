    ...
    const ButtonFollowHandler = async (isbn_code) => {
        try {
            ...
            if(followingsList.length > 0 && followingsList.some(obj => obj.id === product.id)) {
                ...
                document.querySelector(".add-btn-f").innerHTML = t("common:product.buttons.remove");
                if (dispatch && dispatch !== null && dispatch !== undefined) {
                    dispatch(setAlert("Вы подписали", "success"));
                }
            }
                ...
                <div className="product-container">
                    ...
                    :   
                        <div className="fist-about">
                            ...
                            <button onClick={() => ButtonFollowHandler(product.isbn_code)} className="add-btn-f" title="Подписаться">{t("common:product.buttons.add")}</button>
                        </div>
                    }
