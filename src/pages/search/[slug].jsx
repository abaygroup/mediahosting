...
const CategoryDetail = ({data, access}) => {
    const router = useRouter();
    const { sub_category, products } = data;
    const [favorites, setProducts] = useState(data.favorites || []);
        ...
        >
            <div className="category-detail">
                <div className="category-content">
                    <div className="head" style={{ backgroundImage: `url(${sub_category.image})`}}>
                    ...
                    {products.map((product, i) => (
                        <div className="product-box" key={i}>
                            ...
                            <button className="addFav" onClick={() => addToFavorite(product.isbn_code)}>
                                {favorites.length > 0 && favorites.some(obj => obj.id === product.id) ?
...
export async function getServerSideProps(context) {
    ...
    const response = await fetch(`${BACKEND_URL}/api/category/${context.params.slug}/`, context.req.cookies.access && config);
    const data = await response.json()
